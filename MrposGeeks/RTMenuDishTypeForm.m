...
@implementation RTMenuDishTypeForm
...
-(void)defDataLoad{
    NSString *sqlstr = @"";
    sqlstr = [NSString stringWithFormat:@"select * from `dish_type` where `sid`=%zd",self.typeID];
...
- (IBAction)actSave:(id)sender {
    ...
    if ([rtStringCheck checkIsNotNull:strName]) {
        NSString *sqlstr = [NSString stringWithFormat:@"update `dish_type` set `status`=%@,`name`='%@' where `sid`=%zd",strStatus,strName,self.typeID];
